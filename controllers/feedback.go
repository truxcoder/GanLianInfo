...
func FeedbackList(c *gin.Context) {
	var mos []struct {
		...
		FeedbackerName string `json:"feedbackerName"`
		OrganShortName string `json:"organShortName"`
	}
	var mo models.Feedback
	selectStr := "feedbacks.*,personnels.name as personnel_name,per.name as feedbacker_name," +
		"departments.short_name as organ_short_name"
	joinStr := "left join personnels on feedbacks.personnel_id = personnels.id" +
		" left join personnels as per on feedbacks.feedbacker = per.id" +
		" left join departments on departments.id = feedbacks.organ_id"
	getList(c, "feedbacks", &mo, &mos, &selectStr, &joinStr)
}
